...
class OthelloGUI:
    ...
    def run_game(self, auto_close=False, return_winner=False):
        """
        Runs the main game loop, handling events, drawing the board, and managing the game state.

        Parameters:
        auto_close (bool): If True, the window will close automatically at the end of the game.
        """
        running = True
        game_over = False
        ...
        while running:
            ...
            if not game_over and (current_player_valid_moves.size == 0 and opponent_valid_moves.size == 0):
                game_over = True
                if auto_close:
                    running = False  # Automatically close the window when the game is over
