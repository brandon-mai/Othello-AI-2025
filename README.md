...
- `c:` The exploration constant for MCTS.
...
Currently, bitboards are utilized primarily within the heuristic functions to perform calculations efficiently. However, there are plans to extend their usage to other parts of the algorithm in the near future. By incorporating bitboards more extensively, we aim to enhance the overall performance of the algorithm, enabling faster and more optimized decision-making processes across all components.

Additionally, it's worth noting that currently, the algorithms utilizing bitboards are not fully optimized as they largely replicate the algorithms used on the normal arrays. The entirety of these algorithms are currently being redesigned to leverage the full power of bitwise operations.
